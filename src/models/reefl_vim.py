# Taken and adopted from https://github.com/hustvl/Vim/blob/main/vim/models_mamba.py
...
import os
# import sys
# sys.path.append('/home/CORP/royson.lee/fedl2p_private')
# import pdb
...
from src.models.mamba_simple import Mamba
...
class VisionMamba(nn.Module):
    def __init__(self, 
        ...
        assert self.use_middle_cls_token == True
    ...
    def forward(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        if not self.if_bidirectional:
            ...
            output_logits = []
            early_exit_idx = 0
            for blk_idx, layer in enumerate(self.layers):
                ...
                cls_token = hidden_states[:, token_position, :][:, None, :]
                ...
                if blk_idx in self.blks_to_exit:
                    ...
                    if isinstance(self.accumulator, Ree):
                        ...
                        _outputs = self.accumulator.mlp_head(mod_tokens[:,0] + exit_cls_tokens[:, -1])
                ...
                if isinstance(self.accumulator, Ree): 
                    if self.accumulator.modulation:
                        ...
                        hidden_states[:, token_position, :] = mod_tokens[:, -1]
...
def vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(depth, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=192, depth=depth, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, **kwargs)
    model.default_cfg = _cfg()
    return model

def reefl_vim_template(base_model, accumulator, adapter, 
        depth=24,
        no_of_exits=None, 
    ...
    assert adapter is None # peft not supported
    if last_exit_only:
    ...
    assert os.path.exists(f'base_models/{base_model}.pt')
    checkpoint = torch.load(f'base_models/{base_model}.pt')
    ...
    if width_scale == 1:
        ...
        model.load_state_dict(prune(checkpoint_no_module, param_idx), strict=False)
