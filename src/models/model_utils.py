...
def prune(state_dict, param_idx):
    """Prune width and depth of DNN. Return state_dict whose keys are found in param_idx"""
    """Adopted and modified from https://github.com/adap/flower/blob/main/baselines/depthfl/depthfl/client.py"""
    ret_dict = {}
    for k in state_dict.keys():
        if k in param_idx:
            if "num" not in k:
                ret_dict[k] = state_dict[k][torch.meshgrid(param_idx[k], indexing='ij')]
            else:
...
def set_partial_weights(model: torch.nn.ModuleList, update_params: List[str], weights: fl.common.Weights) -> None:
    """Set model weights from a list of NumPy ndarrays."""
    assert len(update_params) == len(weights)
    partial_keys = [k for k in model.state_dict().keys() if k in update_params]
    state_dict = OrderedDict(
        {
            k: torch.Tensor(np.atleast_1d(v))
            for k, v in zip(partial_keys, weights) if k in update_params
        }
    )
