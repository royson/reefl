...
def ree_early_exit_test(net, max_early_exit_layer, testloader, device: str, accuracy_scale=1., ensemble=False):
    """Validate the network on the entire test set."""
    criterion = torch.nn.CrossEntropyLoss()
    net.eval()
    
    results = {}
    for ee in range(max_early_exit_layer + 1):
        results[ee] = {}
        results[ee]['correct'] = 0.0
        results[ee]['total'] = 0.0
        results[ee]['loss'] = 0.0
        if ensemble:
            results['ensemble_all_total'] = 0.
            results['ensemble_all_correct'] = 0.
    ...
    with torch.no_grad():
        for data in testloader:
            ...
            if ensemble:
                ensemble_output = torch.stack(all_outputs, dim=2)
                ensemble_output = torch.sum(ensemble_output, dim=2) / len(ensemble_output)
                _, predicted = torch.max(ensemble_output, 1)
                results['ensemble_all_total'] += labels.size(0)
                results['ensemble_all_correct'] += (predicted == labels).sum().item()

            for ee, outputs in enumerate(all_outputs):
                results[ee]['loss'] += criterion(outputs, labels).item() * labels.size(0)
                _, predicted = torch.max(outputs.data, 1)
                results[ee]['total'] += labels.size(0)
                results[ee]['correct'] += (predicted == labels).sum().item()
    
    if ensemble:
        results['ensemble_accuracy'] = results['ensemble_all_correct'] / results['ensemble_all_total']

    for ee in range(max_early_exit_layer + 1):
        results[ee]['accuracy'] = (results[ee]['correct'] / results[ee]['total']) * accuracy_scale
        results[ee]['loss'] = results[ee]['loss'] / results[ee]['total']
